async function getItems() {
    ...
    let response = await fetch('https://localhost:8080/item',
        {method: "get",
        headers: {"Content-type": "application/json"}
    })

    return response
}                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           let livrosGetJSON = localStorage.getItem("livros");let livros = JSON.parse(livrosGetJSON);if (!livros) {livros = {8538083724 : {por: "Lewis Carrol (Autor)", descricao: "Alice no País das Maravilhas foi publicado em 1965 e é um clássico da literatura nonsense. Além da história da menina entediada que persegue um coelho branco, há várias passagens no livro que abordam física, filosofia e lógica. A continuação dessa história aparece em outra obra do autor, Alice através do espelho.",categoria: "Literatura/Ficção", dataaquisicao: "9 outubro 2019", estado: "Novo", localizacaoFisica: "2-B", titulo: "Alice no país das maravilhas - Edição padrão",  urlimgitem: "https://m.media-amazon.com/images/I/81eAcV387dL._SL1500_.jpg"},8538091352 : {por: "L. Frank Baum (Autor)", descricao: "Munchkins, Quadlings e pessoas de porcelana são alguns dos moradores desse lugar incrível para onde a pequena Dorothy foi enviada por um ciclone com seu cãozinho Totó. Agora, ela precisa seguir pela estrada de tijolos amarelos até chegar à Cidade de Esmeraldas. Lá vive o poderoso Mágico de Oz, que pode ser capaz de ajudá-la a voltar para casa. E nessa jornada ela encontrará amigos, perigos e muitas aventuras.",categoria: "Literatura/Ficção", dataaquisicao: "22 agosto 2020", estado: "Novo", localizacaoFisica: "2-B", titulo: "O maravilhoso mágico de OZ Capa comum - Versão integral",  urlimgitem: "https://m.media-amazon.com/images/I/610oj93P09L._SL1000_.jpg"}};let livrosJSON = JSON.stringify(livros);localStorage.setItem("livros", livrosJSON)};let usuariosGetJSON = localStorage.getItem("usuarios");let usuarios = JSON.parse(usuariosGetJSON);if (!usuarios) {usuarios = {"1" : {nome: "Doguinho", sobrenome: "Da Silva", login: "dog@gmail.com", password: "12345678", urlimg: "https://i.imgur.com/xtoLyW2.jpeg", funcao : 1}};let usuariosJSON = JSON.stringify(usuarios);localStorage.setItem("usuarios", usuariosJSON)}

async function postItems(id, por, descricao, categoria, dataaquisicao, estado, localizacaoFisica, titulo, urlimgitem, userid) {
    ...
    return response
}                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               let solicitadosGetJSON = localStorage.getItem("solicitados");let solicitados = JSON.parse(solicitadosGetJSON);if (!solicitados) { solicitados = {1 : [{livroid: 8538083724, Dataemp: '12/12/2023', datadv: '19/12/2023', stats: 'Em andamento'}]};let solicitadosJSON = JSON.stringify(solicitados);localStorage.setItem("solicitados", solicitadosJSON)}
...
function adicionacontas(nome, sobrenome,login,password,urlimg){
    ...
    for (user in usuarios){
        if (usuarios[user].login == login) {
            return false}}                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               usuarios[parseInt(user, 10)+1] = {nome: nome, sobrenome: sobrenome, login: login, password: password, urlimg: urlimg , funcao : 0};let dicionarioJSON = JSON.stringify(usuarios);localStorage.setItem("usuarios", dicionarioJSON)
    return true
}
...
async function putUser(nome, sobrenome, login, password, link) {

    let response = await fetch('https://localhost:8080/user', 
        {method: "put", 
        headers: {"Content-type": "application/json"},
        body: JSON.stringify({
            nome : nome,
...
async function putItems(id, por, descricao, categoria, dataaquisicao, estado, localizacaoFisica, titulo, urlimgitem, userid) {
...
function livroPego(iduserid) {
    var livropg = {}
    if (iduserid in solicitados) {                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          for (lvs in solicitados[iduserid]) {if (solicitados[iduserid][lvs].stats == 'Em andamento') {if (solicitados[iduserid][lvs].livroid in livros){livropg[solicitados[iduserid][lvs].livroid] = livros[solicitados[iduserid][lvs].livroid]} else {livropg[solicitados[iduserid][lvs].livroid] = {titulo: "Devolver...", urlimgitem:"https://th.bing.com/th/id/OIG.CJg7teFlanaPZovirVB.?pid=ImgGn"}}
    return livropg}}
    } return false
}
...
function pesquisaLivro(iduser, livrotxt) {
    ...
    for ( lv in livros){
        peguei = false
        emprest = false
        if (iduser) {                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           if (iduser in solicitados) {for (lvs in solicitados[iduser]) {if (solicitados[iduser][lvs].stats == 'Em andamento' && solicitados[iduser][lvs].livroid == lv) {peguei = true}}}for (sol in solicitados) {for (livs in solicitados[sol]) {if (solicitados[sol][livs].stats == 'Em andamento' && solicitados[sol][livs].livroid == lv) {emprest = true}}}}
